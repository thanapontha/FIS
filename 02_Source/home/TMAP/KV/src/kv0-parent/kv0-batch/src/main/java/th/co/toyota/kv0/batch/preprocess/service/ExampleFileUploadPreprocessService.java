 ...
 * Client Name				:  TDEM
 * Package Name             :  th.co.toyota.kv0.batch.preprocess.service
 ...
 ********************************************************/
package th.co.toyota.kv0.batch.preprocess.service;
...
import th.co.toyota.kv0.batch.preprocess.repository.ExampleFileUploadPreprocessRepository;
import th.co.toyota.kv0.batch.preprocess.vo.UploadParamVO;
...
//	protected String projectCode;
	...
	public int validate(Connection conn, CST31250FileReceivingCmdOptions params) {
		...
		try {
			...
			logger.error(errMsg);
			loggerBKV02130.error(paramVo.getAppId(), e.getMessageCode(), errMsg, paramVo.getUserId());
		...
		try {
			...
			for (int iRowStage = 0; iRowStage < objList.size(); iRowStage++) {
				...
				if(!duplicate){
					...
					if (!validateLogError(errMsg)) {
						logger.error(errMsg);
						loggerBKV02130.error(this.getParamVo().getAppId(), MessagesConstants.B_ERROR_DUPLICATE_FOUND, errMsg, this.getParamVo().getUserId());
					}
					error = true;
				}
				if(error){
			...
			String errMsg = messageSource.getMessage(e.getMessageCode(), e.getMessageArg(), Locale.getDefault());
			logger.error(errMsg);
			loggerBKV02130.error(this.getParamVo().getAppId(), e.getMessageCode(), errMsg, this.getParamVo().getUserId());
			return CST30000Constants.ERROR;
		}
		if(this.errorCnt > 0) {
			return CST30000Constants.ERROR;
	...
	private void loggedMandatoryError(StringBuilder errValues, int row){
		...
		if (!validateLogError(errMsg)) {
			logger.error(errMsg);
			loggerBKV02130.error(this.getParamVo().getAppId(), MessagesConstants.B_ERROR_INVALID_EMPTY_FIELD, errMsg, this.getParamVo().getUserId());
		}
	}
	...
	public Object[] manageTransaction(Connection conn) {
		...
		try {
			...
			if (!validateLogError(errMsg)) {
				loggerBKV02130.error(this.getParamVo().getAppId(), CST30000Messages.ERROR_UNDEFINED_ERROR, errMsg, this.getParamVo().getUserId());
			}
			result[0] = CST30000Constants.ERROR;
			result[1] = new HashMap<String, String>();
			result[2] = "";

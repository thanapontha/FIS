 ...
 ********************************************************/
package th.co.toyota.kv0.batch.report.main;
...
import th.co.toyota.kv0.api.common.CommonPOI;
...
import th.co.toyota.kv0.api.repository.common.SystemMasterAPIRepository;
import th.co.toyota.kv0.batch.report.repository.CommonDownloadReportRepository;
...
public class CommonDownloadReport {
	...
	protected IST30000LoggerDb loggerBKV04221;
	...
	private final String reportId = "LKV04221";
	...
	public List<String> generateReport(String[] args) throws CommonErrorException {
		...
		try {
			...
			logger.info(msg);
			loggerBKV04221.start(appId, CST30000Messages.INFO_PROCESS_START, msg, userLog);
			...
			loggerBKV04221.error(appId, e.getMessageCode(), message, this.userLog);
			...
			logger.error(message);
			loggerBKV04221.error(appId, CST30000Messages.ERROR_UNDEFINED_ERROR, message, this.userLog);
			status = CST30000Constants.ERROR;
		}finally{
			if (status == CST30000Constants.ERROR) {
				...
				logger.info(message);
				loggerBKV04221.endError(appId,CST30000Messages.INFO_PROCESS_END_ERROR, message, this.userLog);
				status = CST30000Constants.ERROR;
				...
				logger.info(message);
				loggerBKV04221.end(appId, CST30000Messages.INFO_PROCESS_END_SUCCESS, message, this.userLog);
				status = CST30000Constants.SUCCESS;

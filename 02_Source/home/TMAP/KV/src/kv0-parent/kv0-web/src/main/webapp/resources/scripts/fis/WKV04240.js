(function($){		
	...
	window.searchFinish =
		...
		function disabledOperationButton(flag){	
			ST3Lib.validate.disabledButtonSpecify('#WKV04240Download', flag);	
			ST3Lib.validate.disabledButtonSpecify('#WKV04240Browse', flag);	
			ST3Lib.validate.disabledButtonSpecify('#WKV04240Upload', flag);	
			ST3Lib.validate.disabledButtonSpecify('#WKV04240Edit', flag);
		}
	...
	window.saveAddEditObject = 
		function saveAddEditObject(){
			ST3Lib.dialog.confirm(MKV00001ACFM, 'MKV00001ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
		            ...
		            success: function(data) {
		            	if(data  && typeof data != 'undefined'){	
			        		...
			        		if(objSerialize.errorMessages != "" && typeof objSerialize.errorMessages != 'undefined') {
				            	...
				            	$('#WKV04240Upload').removeAttr('disabled');
				            	loadingUpload.close();
				            	loadingUpload = null;
				            	disabledOperationButton(false);
				            	checkConfirmMessageBeforeExit = false;

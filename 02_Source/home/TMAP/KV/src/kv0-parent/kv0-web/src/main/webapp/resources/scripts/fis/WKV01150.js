(function($){
			...
			if(dataList){
				...
				for(var j = 0; j < processPerTick; ++j) {
					...
					if ( data ) {
						...
						FISLib.selectOneCheckboxMode('#result');
	...
	function enableButtonBaseOnStatus(selectedcheckbox){
		...
		if(selectedRow && checked){
			...
			ST3Lib.validate.disabledButtonSpecify( '#WKV01150Download', false );
			
			if(status === '-' || status ==='MP' || status === 'SM' || status === 'PC' || status ==='CP'){
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Upload', true );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Browse', true );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Reset', true );
			}else{
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Upload', false );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Browse', false );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01150Reset', false );
			}
			
			if(status ==='WE' || status === 'WS' || status === 'WS-LT0'){
				ST3Lib.validate.disabledButtonSpecify('#WKV01150Log', false);
			}else{
				ST3Lib.validate.disabledButtonSpecify('#WKV01150Log', true);
			}
			if(status ==='K-FX' || status === 'R-FX' || status === 'S-FX' || status === 'WS-LT0'){
				if(confirmedStatus === 'YES'){
					ST3Lib.validate.disabledButtonSpecify('#WKV01150Complete', false);
				}else{
					ST3Lib.validate.disabledButtonSpecify('#WKV01150Complete', true);
				}				
			}else{
				ST3Lib.validate.disabledButtonSpecify('#WKV01150Complete', true);
			}
			...
			if(status ==='CP' &&  $("#getsudoMonthSearch").val() === currentMonthYear){
				ST3Lib.validate.disabledButtonSpecify('#WKV01150CompletedReset', false);
			}else{
				ST3Lib.validate.disabledButtonSpecify('#WKV01150CompletedReset', true);
			}
		}else{
			ST3Lib.validate.disabledButtonSpecify( '#WKV01150Download', true );
			ST3Lib.validate.disabledButtonSpecify( '#WKV01150Upload', true );
			ST3Lib.validate.disabledButtonSpecify('#WKV01150Browse', true);
			ST3Lib.validate.disabledButtonSpecify('#WKV01150Log', true);
			ST3Lib.validate.disabledButtonSpecify('#WKV01150Complete', true);
			...
			if (formValidateError(searchForm) && formValidateError(uploadForm)) {
				...
				if(selectedUpdateKeySet){
					...
					if(workSheetStatus === 'WS'){
						...
						MKV00005ACFM_Upload = MKV00005ACFM_Upload.replace("{Date_Time}", updateDtPrev);
						ST3Lib.dialog.confirm(MKV00005ACFM_Upload, 'MKV00005ACFM', function(ret){
							if (!ret) {
								return;
							}else{
								uploadWorksheet();
            ...
            success: function(data) {
            	if(data  && typeof data != 'undefined'){	
	        		...
	        		if(objSerialize.errorMessages != "" && typeof objSerialize.errorMessages != 'undefined') {
		            	...
		            	$('#WKV01150Upload').removeAttr('disabled');
		            	loadingUpload.close();
		            	loadingUpload = null;
		            	enableButtonBaseOnStatus(selectedCheckbox);
		            	checkConfirmMessageBeforeExit = false;
	...
	window.doComplete = 
		function doComplete(){
			$('#WKV01150Browse').val('');
			ST3Lib.dialog.confirm(MKV00004ACFM_Complete, 'MKV00004ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.doReset = 
		function doReset(){
			$('#WKV01150Browse').val('');
			ST3Lib.dialog.confirm(MKV00004ACFM_Reset, 'MKV00004ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.doCompletedReset = 
		function doCompletedReset(){
			$('#WKV01150Browse').val('');
			ST3Lib.dialog.confirm(MKV00006ACFM_CompleteReset, 'MKV00006ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();

(function($) {
	...
	$(function() {
		...
		ST3Lib.validate.disabledButtonSpecify('#WKV02111Save', true);	
	...
	window.doSave = function doSave() {
		ST3Lib.dialog.confirm(MKV00004ACFM_ROUND_RESULT, 'MKV00004ACFM', function(ret){
			if (!ret) {
				return;
			}else{
				...
				$.ajax({
	            	...
	            	success: function(data) {
		            	...
		            	ST3Lib.validate.disabledButtonSpecify('#WKV02111Save', true);
	    				ST3Lib.validate.disabledButtonSpecify('#WKV02111Round', false);
		            },		
		        	error: function() {
		        		ST3Lib.validate.disabledButtonSpecify('#WKV02111Save', true);
	    				ST3Lib.validate.disabledButtonSpecify('#WKV02111Round', false);
	...
	window.doRound = function doRound() {
		...
		if(formCustomValidateError(saveForm, lotsize)){
			...
			if(doSave){
				...
				ST3Lib.validate.disabledButtonSpecify('#WKV02111Save', false);
				ST3Lib.validate.disabledButtonSpecify('#WKV02111Round', true);
			}else{
				ST3Lib.validate.disabledButtonSpecify('#WKV02111Save', true);
				ST3Lib.validate.disabledButtonSpecify('#WKV02111Round', false);
			}

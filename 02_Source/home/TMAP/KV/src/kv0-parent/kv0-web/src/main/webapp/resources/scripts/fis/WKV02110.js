...
//			enableButtonBaseOnStatus(selectedCheckbox);
	...
	function enableButtonBaseOnStatus(){
		...
		if(iSelectedCount > 1){
			...
			if(isMultiVehiclePlant || isMultiUnitPlant || isSelectFix || isCannotValidate) {
				...
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Browse', true);
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Validate', true);
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Detail', true);
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Log', true);
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Fix', true);
			}else{
				//Disable PAMs/Skip upload/Detail/Log/Fix
				$("input[type=radio]").attr('disabled', true);
				...
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Browse', false);
				ST3Lib.validate.disabledButtonSpecify('#WKV02110Validate', false);
			}
			...
			if(selectedRow && checked){
						...
						"S-FX" == status ) //Skip Upload Fixed
				{
					...
					ST3Lib.validate.disabledButtonSpecify('#WKV02110Browse', false);
					ST3Lib.validate.disabledButtonSpecify('#WKV02110Validate', false);
					ST3Lib.validate.disabledButtonSpecify('#WKV02110Detail', false);
					ST3Lib.validate.disabledButtonSpecify('#WKV02110Log', false);
					if ("Y" == roundFlag) {
						ST3Lib.validate.disabledButtonSpecify('#WKV02110Fix', false);
					} else {
						ST3Lib.validate.disabledButtonSpecify('#WKV02110Fix', true);
					}
			...
			if (status != "") {
				if ("R-SC" == status) {
					if (uploadTypeChecked.trim() == "K") {
						ST3Lib.message.addError(MKV00014AERR);
						ST3Lib.message.show(1);
						return false;
					}
				}
	...
	window.doDetail = function doDetail() {
		...
		if('K' == uploadType && 'SC' == uploadStatus){
			FISLib.dialog.open("VanningAdjustmentDialog", _rootPath
					...
					+ "&unitPlant=" + unitPlant
					+ "&unitType=" + unitType,
					"WKV02111 : Vanning Adjustment Screen", 1000, 500);
	...
	window.doDownloadDetailReport =
		function doDownloadDetailReport(uploadType){
			...
			$('#WKV02110Browse').removeClass('MandatoryField');
			if (formValidateError(searchForm) && formValidateError(uploadForm)) {
				$('#WKV02110Browse').addClass('MandatoryField');
...
//						enableButtonBaseOnStatus(selectedCheckbox);
	...
	window.doFix = function doFix() {
		FISLib.clearST3Message();
		$('#WKV02110Browse').val('');
		ST3Lib.dialog.confirm(MKV00004ACFM_FIX, 'MKV00004ACFM', function(ret){
			if (!ret) {
				return;
			}else{
				var loadingTarget = "#screen-panel";
				...
				.done(
					function(data1) {
						...
						if (typeof objPayload.status != 'undefined') {
							...
							document.getElementById("upload-form").reset();
							$('#WKV02110Upload').removeAttr('disabled');

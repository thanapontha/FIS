(function($){
	...
	window.enableButton = 
		...
		if(mode == 'Initial'){
			...
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Download', true );
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Browse', true );
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Upload', true );
			
			ST3Lib.validate.disabledButtonSpecify('#WKV02160CheckingReport', true);
			ST3Lib.validate.disabledButtonSpecify('#WKV02160Confirm', true);
		}else if(mode == 'Search'){
			$('#result input:checkbox:first').focus();
			
			ST3Lib.content.disabled( '#parentLineUpload', false );
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Download', false );
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Browse', false );
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Upload', false );
			
			ST3Lib.validate.disabledButtonSpecify('#WKV02160CheckingReport', false);
			ST3Lib.validate.disabledButtonSpecify('#WKV02160Confirm', true);
			...
			ST3Lib.validate.disabledButtonSpecify( '#WKV02160Upload', false );
			ST3Lib.validate.disabledButtonSpecify('#WKV02160CheckingReport', false);
			ST3Lib.validate.disabledButtonSpecify('#WKV02160Confirm', false);
	...
	window.doConfirm = 
		...
		$('#WKV02160Browse').val('');
		ST3Lib.dialog.confirm(MKV00004ACFM_CONFIRM, 'MKV00004ACFM', function(ret){
			if (!ret) {
				return;
			}else{
	...
	window.doClear =
		...
		window.doDownload =
			function doDownload(){
				...
				$('#WKV02160Browse').removeClass('MandatoryField');
				if (formValidateError(searchForm) && formValidateError(uploadForm)) {
					$('#WKV02160Browse').addClass('MandatoryField');
	            ...
	            success: function(data) {
	            	if(data  && typeof data != 'undefined'){	
		        		...
		        		if(objSerialize.errorMessages != "" && typeof objSerialize.errorMessages != 'undefined') {
			            	...
			            	$('#WKV02160Upload').removeAttr('disabled');
			            	loadingUpload.close();
			            	loadingUpload = null;
			            	enableButton('Search');
			            	checkConfirmMessageBeforeExit = false;
	...
	window.doCheckingReport = 
		...
		FISLib.PageOpen(basePath + '/common/fisCommonDownload', "WKV03210", jsonParams);
	}

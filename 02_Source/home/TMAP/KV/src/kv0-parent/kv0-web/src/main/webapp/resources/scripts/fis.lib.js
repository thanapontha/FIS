...
FISLib.window = window;
FISLib.$ = $;

FISLib.clearST3Message = function() {
	ST3Lib.message.setAppendMode(false);
	ST3Lib.message.clear(true);
	ST3Lib.message.setAppendMode(false);
}

FISLib.slideToggle = function(obj, elem){
		if ((elem) && (obj)) {
			...
			alert('Please specific element id. (FISLib.slideToggle.elementId = "mydialog";)');
		}
};
...
FISLib.dialog = {
	...
	init: function(){
		...
		if(this.elementId != ""){
			...
			alert('Please specific element id. (FISLib.dialog.elementId = "mydialog";)');
		}
	},
...
FISLib.numberWithCommas = function(x, isDecimal) {
...
FISLib.concatFullName = function (title,firstName,lastName) {
		var otitle = '';
		var ofirstName = '';
		var olastName = '';
		if(title != null && title != '') 
...
/*
 * Function FISLib.selectOneCheckboxMode()
 * Parameters:
 * 		id 		= id of element that want to use select only one checkbox in a group
 * 		Sample call FISLib.selectOneCheckboxMode('#result');
 * 		***NOTE: Checkbox must be generate before call this function.***
 */
FISLib.selectOneCheckboxMode = function selectOneCheckboxMode(id){

(function($){
	...
	window.onbeforeunload = onbeforeunloadFIS;
	
	function onbeforeunloadFIS (event) {
	...
	window.diableOnEditMode =
		function diableOnEditMode() {
			...
			ST3Lib.validate.disabledButtonSpecify( '#WKV01110Browse', true );
	...
	function enableButtonBaseOnVersionAndStatus(){
		...
		if(dataNotFound == 'Y'){
			if(version == TDEM){
				jQuery("#WKV01110Submit").hide();
				jQuery("#WKV01110RejectKaikieng").show();
				jQuery("#WKV01110Fix").show();
				
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Upload', true );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Browse', true );
				ST3Lib.validate.disabledButtonSpecify('#WKV01110Edit', true);
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Fix', true );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110RejectKaikieng', true );
				if(kaikiengStatus == 'MP'){
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Download', false );
				}else{
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Download', true );
				}
			}else{
				jQuery("#WKV01110Submit").show();
				jQuery("#WKV01110RejectKaikieng").hide();
				jQuery("#WKV01110Fix").hide();
				
				ST3Lib.validate.disabledButtonSpecify('#WKV01110Edit', false);
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Upload', false );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Browse', false );
				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Download', false );

				ST3Lib.validate.disabledButtonSpecify( '#WKV01110Submit', true );
			}
		}else{
			if(version == TDEM){
				jQuery("#WKV01110Submit").hide();
				...
				if(kaikiengStatus == '' || kaikiengStatus == 'FX'){
					...
					if("YES"===enableFixButton){
						ST3Lib.validate.disabledButtonSpecify( '#WKV01110Fix', false );
					}else{
						ST3Lib.validate.disabledButtonSpecify( '#WKV01110Fix', true );
					}					
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110RejectKaikieng', false );
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Browse', false );
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Upload', false );
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Download', false );
				}
				...
				if(kaikiengStatus == '' || kaikiengStatus == 'SM'){
					...
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Browse', false );
					ST3Lib.validate.disabledButtonSpecify('#WKV01110Edit', false);
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Submit', false );
					ST3Lib.validate.disabledButtonSpecify( '#WKV01110Download', false );
				}
			}
		}
	...
	window.saveAddEditObject = 
		function saveAddEditObject(){
			ST3Lib.dialog.confirm(MKV00002ACFM, 'MKV00002ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.submitKaikieng = 
		function submitKaikieng(){
			$('#WKV01110Browse').val('');
			ST3Lib.dialog.confirm(MKV00004ACFM_SubmitKaikieng, 'MKV00004ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.fixKaikieng = 
		function fixKaikieng(){
			$('#WKV01110Browse').val('');
			ST3Lib.dialog.confirm(MKV00004ACFM_FixKaikieng, 'MKV00004ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.rejectKaikieng = 
		function rejectKaikieng(){
			$('#WKV01110Browse').val('');
			ST3Lib.dialog.confirm(MKV00004ACFM_RejectKaikieng, 'MKV00004ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					FISLib.clearST3Message();
	...
	window.updateObjectFinish =
		...
		if(datas.status === 'OK'){
			...
			$("#search-form").find(".getsudoMonth").css("display", "");
			ST3Lib.dialog.confirm(MKV00006ACFM, 'MKV00006ACFM', function(ret){
				if (!ret) {
					return;
				}else{
					restoreSearchCriteria();
		            ...
		            success: function(data) {
		            	if(data  && typeof data != 'undefined'){	
			        		...
			        		if(objSerialize.errorMessages != "" && typeof objSerialize.errorMessages != 'undefined') {
				            	...
				            	$('#WKV01110Upload').removeAttr('disabled');
				            	loadingUpload.close();
				            	loadingUpload = null;
				            	enableButtonBaseOnVersionAndStatus();
				            	checkConfirmMessageBeforeExit = false;
